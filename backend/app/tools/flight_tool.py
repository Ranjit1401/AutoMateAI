...
class FlightTool(BaseTool):
    name = "flight"
    description = "Searches live flight offers between two supported Indian cities."

    def execute(self, source: str, destination: str) -> list[dict]:
        if source not in AIRPORTS:
            raise ToolError(f"Unsupported source city '{source}'. Supported: {', '.join(AIRPORTS)}")
        if destination not in AIRPORTS:
            raise ToolError(f"Unsupported destination city '{destination}'. Supported: {', '.join(AIRPORTS)}")
        ...
        except Exception as exc:  # noqa: BLE001 — provider errors surface as ToolError
            raise ToolError(f"Flight search failed: {exc}") from exc
